use crate::{methods::{Method, RawMethod}, set::{Set, Card, CardType, SlimCard}};
...
impl<'e, 's> Driver<'e, 's> {
    ...
    pub(crate) fn new_learn(set: &'s mut Set, raw_method: RawMethod, engine: &'e Engine) -> Result<Self> {
    ...
    pub fn first(&mut self) -> Result<Option<SlimCard>> {
        ...
        self.latest_card = Some(card_id.clone());
        self.curr_count += 1;
